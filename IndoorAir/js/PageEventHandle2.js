...
var AQILevel2=["Good","Moderate","UnhealthyForSensitive","Unhealthy","Very Unhealthy","Hazardous"];
var outDataSource=["海淀万柳","徐汇上师大","无锡旺庄"];
var outDataSource2=["HaidianwanLiu","Xuhuishangshida","Wuxiwangzhuang"];
...
function  setInfo(data) {
    ...
    var res=data[stationInfoKey[4]];
    var res1=res.replace("小时"," Hour(s)");
    res=res1.replace("分钟前"," Minute(s) ago");
    x.rows[4].cells[1].innerHTML=res;
    ...
    for(var i=1;i<=3;i++)
    {
        ...
        if(i==3)
        {
            for(var j=0;j<AQILevel.length;j++)
            {
                if(res1==AQILevel[j])
                {
                    y.rows[1].cells[i].innerHTML = AQILevel2[j];
                    y.rows[1].cells[i].style.color=colorLevel[j];
                }
                if(res2==AQILevel[j])
                {
                    y.rows[2].cells[i].innerHTML=AQILevel2[j];
                    y.rows[2].cells[i].style.color=colorLevel[j];
                }
            }
        }
        else
...
function set_24HoursAQI(item) {
    var res=item[stationAQIKey[0]];
    for(var i=0;i<outDataSource.length;i++)
    {
        if(res==outDataSource[i])
            $("#outDataFrom").text(outDataSource2[i]);
    }
